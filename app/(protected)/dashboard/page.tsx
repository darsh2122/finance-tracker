...
// Wraps the heavy async data fetch in Suspense so the skeleton shows instantly.
...
// ── The real data-fetching component ──────────────────────────────────────────
async function DashboardContent() {
  ...
  const { data: txns, error } = await supabase
    .from("transactions")
    .select("id,direction,amount,currency,description,occurred_at,category_id")
  ...
  const categoryIds = Array.from(new Set((txns || []).map((t: any) => t.category_id).filter(Boolean)))
  ...
  const enriched = (txns || []).map((t: any) => {
    ...
    return {
      id: t.id, direction: t.direction, amount: Number(t.amount),
      currency: t.currency ?? baseCurrency, description: t.description ?? "",
...
// ── The exported page — skeleton shows while DashboardContent streams ─────────
